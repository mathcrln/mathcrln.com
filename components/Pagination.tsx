		...
		<div className={`flex justify-between items-center ${className} text-lg`}>
			{hasPreviousPage && (
				<Link href={pageNumber === 2 ? '/blog' : `/blog/page/${pageNumber - 1}`}>
					<a
						className={`group hover:bg-gray-700 focus:bg-gray-700 focus:outline-none items-center flex ${
							verbose ? 'py-2 px-4 rounded-sm' : 'pr-1'
						}`}
			...
			{hasNextPage && (
				<Link href={`/blog/page/${pageNumber + 1}`}>
					<a
						className={`group hover:bg-gray-700 focus:bg-gray-700 focus:outline-none items-center ml-auto flex ${
							verbose ? 'py-2 px-4 rounded-sm' : 'pl-1'
						}`}
						title={`Browse to page ${pageNumber + 1}`}
					>
